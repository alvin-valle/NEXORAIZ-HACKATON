...
- Sirve el frontend estático (`/public`).
...
- **Nueva identidad visual** basada en los colores de tu logo (verde bosque, verde lima, naranja, café, azul) — ver `public/css/variables.css`.
...
- Eliminé la duplicación "Frontend"/"Backend" que en realidad eran el mismo código: ahora hay una sola fuente de verdad (`public/` + `backend/`).
...
Todo el sistema de color está definido como variables CSS en `public/css/variables.css`, con un set alternativo bajo `[data-theme="oscuro"]`. Para ajustar la paleta, ese es el único archivo que necesitas tocar.
...
- **Multimedia**: el proyecto original referenciaba audio/video para las leyendas e idiomas que nunca se incluyeron en el zip. Falta grabar/conseguir esos archivos y colocarlos en `public/audio/` y `public/video/`.
...
Proyecto **NEXORAIZ** — 
Equipo: Pedro Arias Rosales,
 Alvin Gonzalez, Zahir, 
 Maria, Jose.
